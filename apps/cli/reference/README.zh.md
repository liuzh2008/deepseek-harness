...
| Profile | 参数 |
|---|---|
| `web` | `--host`、`--port`、可重复的 `--trusted-host`、`--allow-remote-privileged-methods` |
...
生产 Web 运行器需要已构建的包和前端产物（`pnpm run build`）。默认服务地址是 `http://127.0.0.1:3080`。CLI 目前有意不支持 `--host 0.0.0.0`，并会以用法错误退出；`--trusted-host` 可添加 `/api` 浏览器信任围栏接受的具名 authority。配置平面（settings、credentials、agent-preset 创作、原生对话框）默认即使对受信任 authority 也钉在 loopback；`--allow-remote-privileged-methods` 把这一钉扎放宽到受信任 authority——这是面向"服务于受信任 LAN/VPN、需要远程编辑设置"的部署的显式选择开关（它是可达性授权，不是认证）。
