...
export abstract class AbstractApiClient implements IApiClient {
  ...
  protected mintRpcId(): RpcId {
    // randomUuid keeps this base platform-neutral on every origin: it prefers
    // the native Web API where it exists and falls back to getRandomValues on
    // insecure origins (plain-HTTP LAN), where crypto.randomUUID is absent.
