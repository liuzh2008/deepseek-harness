...
The foot is one row: the `sidebar.settings` seat — ui-settings registers the trigger row and settings panel there — with optional `sidebar.footer.action` entries (e.g. the file browser) beside it. The sidebar shares its column state (`wide`) with both.
