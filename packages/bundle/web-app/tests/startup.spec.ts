...
`)
  writeFileSync(join(dir, 'cordis.yml'), [
    ...
    '    allowRemotePrivilegedMethods: !!js ctx.webStartup.allowRemotePrivilegedMethods',
    '- id: provider',
...
describe('web command-line provider', () => {
  ...
  it('leaves deployment values to each consumer when flags omit them', async () => {
    const { values, observed } = await bootProvider([])
    expect(values).toEqual({ openBrowser: true, trustedHosts: [], allowRemotePrivilegedMethods: false })
  ...
  it('prints its own help and leaves the consumer pending', async () => {
    ...
    expect(observed.out).toContain('--allow-remote-privileged-methods')
